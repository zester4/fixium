...
export async function exportGuideToPDF(guide: RepairGuide): Promise<void> {
  ...
  doc.text('Fixium Repair Guide', margin, yPosition);
  yPosition += 12;
  ...
  doc.text('Generated by Fixium - AI-Powered Repair Guidance', margin, footerY);
  doc.text(new Date().toLocaleDateString(), pageWidth - margin - 20, footerY);

  // Save the PDF
  const fileName = `Fixium-${guide.deviceInfo.model || guide.deviceInfo.category}-Repair-Guide.pdf`;
  doc.save(fileName);
}
